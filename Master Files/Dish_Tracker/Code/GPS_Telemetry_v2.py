...
def move_pointer(tar_spd , ref_az , new_az , dist_1_to_2 , ref_el , new_el , alt_change):  
    ...
    def adjust_speed(r_el, n_el,r_az,n_az,tar_spd,dist_1_to_2,alt_change):
        ...
        if(up is True):
            ...
            speed_el = (2*(4*(int(ang_target_speed_el)))-1) #UP message (odd number)
            # Accomodates for very small speeds and adjust to min speed for stand
            if speed_el < 5:
                speed_el = 5
            ...
            speed_el = (2*(4*(int(ang_target_speed_el))+1)) #DOWN message (even number)
            # Accomodates for very small speeds and adjust to min speed for stand
            if speed_el < 4:
                speed_el = 4     
            # Accomodates for huge speeds and adjusts to max speed for stand  
...
def move_pointer2(tar_spd,ref_az,new_az,dist,ref_el,new_el,alt_diff): 
    ...
    def adjust_move(r_el, n_el,r_az,n_az,tar_spd,dist,alt_diff):
        """Create move mess171.9518372214542age"""
