...
#==================================================================================================================================
#==================================================================================================================================
STAND_PORT = '/dev/ttyUSB0'
STAND_BAUDRATE = 9600
STAND_TIMEOUT = .1

try:
    qpt = serial.Serial(port=STAND_PORT,baudrate = STAND_BAUDRATE,timeout = STAND_TIMEOUT)
    if (qpt.isOpen() == 1):
        print("\nConnected to Stand\n")
except KeyboardInterrupt:
    qpt.close()
    print("\n   Closing from stand connection attempt ... \n\n")
    exit()   
except (IOError):
    print("\n   Check Port for Stand")
    print("   Expected Port is",STAND_PORT,"\n")
    exit()
...
def TCP_connect(TCP_IP_ADDR,TCP_PORT):
    try:
        ...
        sock.close()
        print("\n   Closing from TCP connection attempt ...\n\n")
        exit()
...
def BT_connect(JOYSTICK_ADDR,JOYSTICK_PORT):
    try:
        sock = bluetooth.BluetoothSocket( bluetooth.RFCOMM )
        sock.connect((JOYSTICK_ADDR,JOYSTICK_PORT))
        sock.settimeout(20)
        print ("\nConnected to Bluetooth Module\n")
    except KeyboardInterrupt:
        sock.close()
        print("\n   Closing from bluetooth connection attempt ... \n\n")
        exit()
    except (socket.error,OSError):
        sock.close()
        print("\n   Check IP Address for Bluetooth Socket.")
        print("   Expected IP is",JOYSTICK_ADDR,"\n")
        exit()       
    return(sock)


def read_bt_value(sock):
    stuff = sock.recvall()
    #sock.send('1')
...
def hold_pos(dbm,cli,delay,hold_threshold):
    ...
    while (dbm <= (dbm_hold + hold_threshold)) and (dbm >= (dbm_hold - hold_threshold)):
        dbm_hold = get_dbm(cli)
        print("Within threshold of +", hold_threshold," dBm, -",hold_threshold," dBm. DBM IS:  ", dbm_hold)
        stop_move()
    return(dbm_hold)    
#==================================================================================================================================    
